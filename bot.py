from telegram import Bot
from datetime import datetime, date
import os
import random

def main():
    # =============================
    # Получаем токен и chat_id из переменных окружения
    # =============================
    TOKEN = os.environ["TOKEN"]       # Твой токен бота
    CHAT_ID = int(os.environ["CHAT_ID"])

    bot = Bot(token=TOKEN)

    # =============================
    # Настройки сообщения
    # =============================
    friend_name = "Валентин"
    today = datetime.now().strftime("%d.%m.%Y")
    end_subscription = date(2026, 1, 29)  # Дата окончания абонемента

    # Считаем сколько дней осталось
    days_left = (end_subscription - date.today()).days
    if days_left < 0:
        days_left = 0

    # Список мотивационных фраз
    phrases = [
        "мы все верим в тебя! 💪",
        "не сдавайся и двигайся к цели! 🏋️‍♂️",
        "помни, каждый день — шаг к успеху! 🚀",
        "давай, сегодня твой день в зале! 🔥",
        "не пропускай и оставайся сильным! 💥"
    ]

    # Список эмодзи для украшения
    emojis = ["💪", "🔥", "🏋️‍♂️", "⚡", "🏃‍♂️"]

    # Выбираем случайную фразу и эмодзи
    phrase = random.choice(phrases)
    emoji = random.choice(emojis)

    # Формируем сообщение
    message = (
        f"{friend_name}, сегодня {today}, {phrase} "
        f"До окончания абонемента осталось {days_left} дней. {emoji}"
    )

    # =============================
    # Отправка сообщения в Telegram
    # =============================
    bot.send_message(chat_id=CHAT_ID, text=message)
    print("Сообщение отправлено:", message)

# =============================
# Точка входа
# =============================
if __name__ == "__main__":
    main()
